...
const TableVatTu = (props, ref) => {
  ...
  const onSelectChangeLeft = async (selectedRowKeys, data) => {
    //Check điều kiện vật tư Kỹ thuật cao= True hoặc Stent phủ thuốc= True //check ở tờ điều trị
    if (
      chiDinhTuLoaiDichVu == 210 &&
      data.some((x) => x.kyThuatCao || x.stentPhuThuoc)
    ) {
      const vatTuKTC = data
  ...
  const columnsTableLeft = [
    {
      title: (
        <HeaderSearch
          isTitleCenter={true}
          title={t("khamBenh.donThuoc.tenVatTu")}
        />
      ),
      dataIndex: "ten",
      key: "ten",
    ...
    },
    {
      ...
      align: "right",
      render: (item, data) => {
        return `${data.giaKhongBaoHiem?.formatPrice()}/${data.tenDvtSoCap}`;
      },
    },
    {
      title: (
...
export default forwardRef(TableVatTu);
