...
const DSThuoc = ({ refreshList }) => {
  const { t } = useTranslation();
  const refSuaThongTin = useRef(null);
  const refSettings = useRef(null);

  const { dsThuoc } = useSelector((state) => state.dvNoiTru);

  const onEdit = (record) => () => {
    refSuaThongTin.current &&
      refSuaThongTin.current.show({ data: record }, () => {
        refreshList();
      });
  };
  ...
  const columns = [
    ...
    Column({
      title: t("quanLyNoiTru.dvNoiTru.thanhTien"),
      sort_key: "thanhTien",
      width: "100px",
      dataIndex: "thanhTien",
      key: "thanhTien",
      i18Name: "quanLyNoiTru.dvNoiTru.thanhTien",
      render: (value) => value.formatPrice(),
    }),
    Column({
      title: t("quanLyNoiTru.dvNoiTru.tt30"),
      sort_key: "tenMucDich",
      width: "100px",
      dataIndex: "tenMucDich",
      key: "tenMucDich",
      i18Name: "quanLyNoiTru.dvNoiTru.tt30",
    }),
    Column({
    ...
    Column({
      title: t("quanLyNoiTru.dvNoiTru.tenLieuDung"),
      sort_key: "tenLieuDung",
      width: "160px",
      dataIndex: "tenLieuDung",
      key: "tenLieuDung",
      i18Name: "quanLyNoiTru.dvNoiTru.tenLieuDung",
      render: (item) => item && moment(item).format("DD/MM/YYYY HH:mm:ss"),
    }),
    Column({
  ...
  return (
    <Main noPadding={true}>
      ...
      <SuaThongTinThuoc ref={refSuaThongTin} />
    </Main>
  );
};

export default DSThuoc;
