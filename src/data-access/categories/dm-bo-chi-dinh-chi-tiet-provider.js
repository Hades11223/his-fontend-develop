...
export default {
  ...apiBase.init({ API: DM_BO_CHI_DINH_CHI_TIET }),
  ...
  patch: ({ id, ...rest }) => {
    return new Promise((resolve, reject) => {
      client
        .patch(`${dataPath}${DM_BO_CHI_DINH_CHI_TIET}/${id}`, rest)
        .then((s) => {
          if (s?.data?.code === 0) resolve(s?.data);
          else reject(s?.data);
        })
  ...
  deleteMultiple: (dsIds) => {
    return new Promise((resolve, reject) => {
      client
        .delete(`${dataPath}${DM_BO_CHI_DINH_CHI_TIET}/batch`, { data: dsIds })
        .then((s) => {
          if (s?.data?.code === 0) resolve(s?.data);
          else reject(s?.data);
        })
  ...
  getListByHopDongKskId: ({ ...payload }) => {
    return new Promise((resolve, reject) => {
      client
        .get(combineUrlParams(`${dataPath}${DM_BO_CHI_DINH_CHI_TIET}`, payload))
        .then((s) => {
          if (s?.data?.code === 0) resolve(s?.data);
          else reject(s?.data);
        })
  ...
  batch: (params) => {
    return new Promise((resolve, reject) => {
      client
        .post(`${dataPath}${DM_BO_CHI_DINH_CHI_TIET}/batch`, params)
        .then((s) => {
          if (s?.data?.code === 0) resolve(s?.data);
          else reject(s?.data);
        })
